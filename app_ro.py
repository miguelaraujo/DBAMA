...
def main():
  ...
  try:
      cnx = mysql.connector.connect(user='root', password='msandbox', host='localhost', port=6447, connection_timeout=5)
...
while True:
    ...
    if i % 15 == 0:
      # Print header
      print("TIME\t\tCONNECTION\t\tDB SERVER\t\tREAD PERFORMED\t\tDATA")
    ...
    print("{0}\t\t{1}\t\t{2}\t\t{3}\t\t\t{4}".format(current_time, result[0], result[1], result[2], result[3]))
    time.sleep(1)
